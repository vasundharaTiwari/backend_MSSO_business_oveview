...
public interface RepoMssoDeposit extends JpaRepository<MssoDeposit, Long> {
    ...
    @Query(value = """
             Select branch_code,report_date, round((sb)::numeric,2) as sb,round((Ca)::numeric,2) as Ca,round((td)::numeric,2) as td, round((deposit)::numeric,2) as deposit\s
                                                                                                                                                                         from advances.MSSO_DEPOSIT where report_date= TO_DATE(:report_date,'DD-MON-YYYY') AND REGION='AURANGABAD'  ORDER BY branch_code
            """, nativeQuery = true)
    public List<DtoMssoBusinessBranchwise> getDepositHOBranchwise(@Param("roname") String roname, @Param("report_date") String report_date);
       ...
       @Query(value = """
            ...
            """, nativeQuery = true)
       public List<DtoMssoBusinessRegionwise> getDepositHORegionwise(@Param("report_date") String report_date);
}
